...
public class SelectiveColorYellowsManipulatorForBluesTask2 extends Task<int[]> {
   ...
   private int currentGreenValue ;
	...
	public SelectiveColorYellowsManipulatorForBluesTask2( Canvas canvas , int currentGreenValue ,  int[] actualRed , int[] actualGreen , int[] actualBlue 
            , double[] actualBlueInFraction , int[] differenceBetweenActualBlueAndActualGreen ) {
		...
		for( int y = 0 ; y < height ; y++ ) {
			for( int x = 0 ; x < width ; x++ ) {
				...
				if( actualBlue[i] > actualGreen[i] && actualGreen[i] > actualRed[i] ) {
					...
					currentBlue = (differenceBetweenActualBlueAndActualGreen[i]*actualBlueInFraction[i])*(currentGreenValue/(double)100) ;
