...
public class SelectiveColorCyansManipulatorForWhitesTask extends Task<int[]> {
   ...
   private int currentRedValue ;
	...
	public SelectiveColorCyansManipulatorForWhitesTask( Canvas canvas , int currentRedValue ,  int[] actualRed , int[] actualGreen , int[] actualBlue 
            , double[] actualRedInFraction , double[] counterRedInFraction , int[] differenceBetweenActualRedAndActualGreen ) {
	...
	protected int[] call() {
		...
		for( int y = 0 ; y < height ; y++ ) {
			for( int x = 0 ; x < width ; x++ ) {
				...
				if( actualRed[i] > 127 && actualRed[i] > actualGreen[i] && actualRed[i] > actualBlue[i] ) {
