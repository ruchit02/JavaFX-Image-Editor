...
public class ChannelMixerBluePixelManipulatorForRedChannelTask extends Task<int[]> {
   ...
   private int currentBlueValue ;
	...
	public ChannelMixerBluePixelManipulatorForRedChannelTask( Canvas canvas , int currentBlueValue ,  int[] actualRed , int[] actualGreen , int[] actualBlue ) {
	...
	protected int[] call() {
		...
		for( int y = 0 ; y < height ; y++ ) {
			for( int x = 0 ; x < width ; x++ ) {
				...
				newRed = (int)(actualRed[i] + ((currentBlueValue/(double)100)*actualBlue[i])) ;
