...
public class ChannelMixerRedPixelManipulatorForBlueChannelTask extends Task<int[]> {
   ...
   private int currentRedValue ;
	...
	public ChannelMixerRedPixelManipulatorForBlueChannelTask( Canvas canvas , int currentRedValue ,  int[] actualRed , int[] actualGreen , int[] actualBlue ) {
	...
	protected int[] call() {
		...
		for( int y = 0 ; y < height ; y++ ) {
			for( int x = 0 ; x < width ; x++ ) {
				...
				newBlue = (int)(actualBlue[i] + ((currentRedValue/(double)100)*actualRed[i])) ;
