...
public class ChannelMixerBluePixelManipulatorForGreenChannelTask extends Task<int[]> {
   ...
   private int currentBlueValue ;
	...
	public ChannelMixerBluePixelManipulatorForGreenChannelTask( Canvas canvas , int currentBlueValue ,  int[] actualRed , int[] actualGreen , int[] actualBlue ) {
	...
	protected int[] call() {
		...
		for( int y = 0 ; y < height ; y++ ) {
			for( int x = 0 ; x < width ; x++ ) {
				...
				newGreen = (int)(actualGreen[i] + ((currentBlueValue/(double)100)*actualBlue[i])) ;
