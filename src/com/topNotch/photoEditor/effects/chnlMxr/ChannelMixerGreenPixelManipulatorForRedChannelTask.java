...
public class ChannelMixerGreenPixelManipulatorForRedChannelTask extends Task<int[]> {
   ...
   private int currentGreenValue ;
	...
	public ChannelMixerGreenPixelManipulatorForRedChannelTask( Canvas canvas , int currentGreenValue ,  int[] actualRed , int[] actualGreen , int[] actualBlue ) {
	...
	protected int[] call() {
		...
		for( int y = 0 ; y < height ; y++ ) {
			for( int x = 0 ; x < width ; x++ ) {
				...
				newRed = (int)(actualRed[i] + ((currentGreenValue/(double)100)*actualGreen[i])) ;
