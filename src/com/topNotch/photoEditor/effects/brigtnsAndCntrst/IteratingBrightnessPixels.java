...
public class IteratingBrightnessPixels extends Thread {
	...
	IteratingBrightnessPixels( int currentBrightnessValue ){
		
		this.currentBrightnessValue = currentBrightnessValue ;
	}
	...
	public void run() { 
		...
		for( int i : elements ) {
			...
			postModificationRED[i] 		= redValue + (int)(currentBrightnessValue*preCalcValues[redValue]) ;
			postModificationGREEN[i] 	= greenValue + (int)(currentBrightnessValue*preCalcValues[greenValue]) ;
			postModificationBLUE[i] 	= blueValue + (int)(currentBrightnessValue*preCalcValues[blueValue]) ;
