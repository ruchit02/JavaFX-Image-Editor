...
public class BlackAndWhiteCyansManipulatorTask extends Task<int[]> {
   ...
   private int currentValue ;
	...
	public BlackAndWhiteCyansManipulatorTask( Canvas canvas , int currentValue ,  int[] actualRed , int[] actualGreen , int[] actualBlue ) {
			...
			for( int x = 0 ; x < width ; x++ ) {
				...
				if( actualGreen[i] >= actualRed[i] && actualBlue[i] >= actualRed[i] ) { 
					...
					if( actualGreen[i] >= actualBlue[i] ) {
						...
						resultant = (int)(differenceBetweenBlueAndRed*(currentValue/(double)100)) ;
						
						newRed = actualRed[i] + resultant ;
						newBlue = newRed ;
						newGreen = newRed ;
					}
					else {
						...
						resultant = (int)(differenceBetweenGreenAndRed*(currentValue/(double)100)) ;
