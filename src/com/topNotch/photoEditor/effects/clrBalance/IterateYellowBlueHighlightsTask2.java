...
public class IterateYellowBlueHighlightsTask2 extends Task<int[]>{
	...
	public IterateYellowBlueHighlightsTask2( Canvas canvas , int currentRedValue ,  int[] actualRed , int[] actualGreen , int[] actualBlue 
	...
	protected int[] call() {
		...
		for( int y = 0 ; y < height ; y++ ) {
			for( int x = 0 ; x < width ; x++ ) {
				...
				int alpha = 0xff<<24 ;
				///
                if( actualRed[i] >= 256 + currentRedValue ) {
					
					newRed = 255 ;
				}
				else {
					
					currentRed = (int)((currentRedValue/(255-(double)actualRed[i]))*actualRed[i]) ; 
					newRed = actualRed[i] - currentRed ;
				}
				
				/////
                if( actualGreen[i] >= 256 + currentRedValue ) {
					
					newGreen = 255 ;
				}
				else {
					
					currentGreen = (int)((currentRedValue/(255-(double)actualGreen[i]))*actualGreen[i]) ; 
				...
				iterations[i] = alpha | newRed<<16 | newGreen<<8 | actualBlue[i] ;
